...
function getSyncFile() {
  return process.env.SYNC_FILE || (process.env.DATA_DIR ? path.join(process.env.DATA_DIR, 'appointments.json') : path.join(__dirname, '..', 'sync', 'appointments.json'));
}

function getDataDir() {
  return process.env.DATA_DIR || path.join(__dirname, '..', 'sync');
}
