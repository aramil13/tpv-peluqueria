...
function dedupAppointments(appts) {
  ...
  (Array.isArray(appts) ? appts : []).forEach(a => {
    ...
    const key = String(a.clientId || '') + '|' + String(a.date || '') + '|' + String(a.time || '') + '|' + String(a.employeeId || '') + '|' + String(a.blockGroupId || '') + '|' + String(a.blockNum || '');
    const existing = seen.get(key);
    if (!existing) { seen.set(key, a); return; }
    const rank = (x) => (isCancelled(x) ? 2 : 1) * 10000000000000 + (x._modified || 0);
