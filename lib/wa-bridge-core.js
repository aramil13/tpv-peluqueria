  ...
  async function checkConfirmedAppointments() {
    ...
    try {
      ...
      for (const appt of appts.filter(a => a._whatsappCancelRejectedPending && !a._whatsappCancelRejectedSent)) {
        ...
        const svcNames = svcIds.map(sid => ((data.services||[]).find(s => s.id === sid))?.name).filter(Boolean).join(', ') || 'Servicio';
        await sendApptNotification(appt, data, {
          text: `ℹ️ Cancelación RECHAZADA:\n\n📅 ${appt.date.split('-').reverse().join('-')}\n⏰ ${appt.time}${appt.endTime ? ' - '+appt.endTime : ''}\n💇 ${svcNames}\n\nLa cita sigue activa. Contacta con el salón si necesitas ayuda.`,
          clearFlag: '_whatsappCancelRejectedPending', setFlag: '_whatsappCancelRejectedSent'
        });
      }
      for (const appt of appts.filter(a => a.cancelledBy === 'salon' && !a._cancelledBySalonSent)) {
        const svcIds = appt.serviceIds || (appt.serviceId ? [appt.serviceId] : []);
        const svcNames = svcIds.map(sid => ((data.services||[]).find(s => s.id === sid))?.name).filter(Boolean).join(', ') || 'Servicio';
        await sendApptNotification(appt, data, {
      ...
      for (const appt of appts.filter(a => a.clientModified && !a.pendingSalonConfirm && !a._whatsappModifiedSent && !a._deleted)) {
        const svcIds = appt.serviceIds || (appt.serviceId ? [appt.serviceId] : []);
        const svcNames = svcIds.map(sid => ((data.services||[]).find(s => s.id === sid))?.name).filter(Boolean).join(', ') || 'Servicio';
        const newDate = appt.pendingDate || appt.date;
        const newTime = appt.pendingTime || appt.time;
        await sendApptNotification(appt, data, {
          text: `✅ Tu solicitud de modificación ha sido ACEPTADA:\n\n📅 ${newDate.split('-').reverse().join('-')}\n⏰ ${newTime}${appt.endTime ? ' - '+appt.endTime : ''}\n💇 ${svcNames}\n\n¡Te esperamos!`,
          clearFlag: 'clientModified', setFlag: '_whatsappModifiedSent'
        });
      }
    } catch {}
  }
