...
const DEFAULT_HOURS = {
  ...
  "2": { open: "09:00", close: "20:30", closed: false, breakStart: "13:30", breakEnd: "15:00" },
  "3": { open: "09:00", close: "20:30", closed: false, breakStart: "13:30", breakEnd: "15:00" },
  "4": { open: "09:00", close: "20:30", closed: false, breakStart: "13:30", breakEnd: "15:00" },
  "5": { open: "09:00", close: "20:30", closed: false, breakStart: "13:30", breakEnd: "15:00" },
  "6": { open: "09:00", close: "13:30", closed: false }
};
...
function getOpeningHoursForDay(dateStr, settings) {
  ...
  return {
    ...
    breakStart: day.breakStart ? parseTime(day.breakStart) : null,
    breakEnd: day.breakEnd ? parseTime(day.breakEnd) : null
  };
}
...
function buildSystemPrompt(data, clientInfo) {
  ...
  const hoursStr = DAY_NAMES.map((name, d) => {
    const day = hours[d] || { open: '09:00', close: '20:30', closed: false };
    if (day.closed) return `${name}: CERRADO`;
    if (day.breakStart && day.breakEnd) {
      return `${name}: ${day.open} - ${day.breakStart} y ${day.breakEnd} - ${day.close}`;
    }
...
INSTRUCCIONES:
- Para buscar servicios: usa searchService("palabras clave"). Ej: searchService("corte mujer") o searchService("mechas").
- checkAvailability(ID_servicio, fecha): cuando tengas servicio + fecha.
- Muestra huecos al cliente con horas y nombres de empleada. NUNCA digas IDs.
- bookAppointment(ID_servicio, fecha, hora, ID_empleado, nombre_cliente): cuando el cliente confirme todo.
- NUNCA digas "reservado" sin bookAppointment exitoso.
- getMyAppointments(): muestra las citas del cliente. Devuelve IDs que usas para cancelar/modificar.
- cancelAppointment(ID_cita): cancela una cita del cliente.
- modifyAppointment(ID_cita, ...): modifica fecha/hora/empleado de una cita. La modificación queda pendiente de confirmación del salón.
- El TELÉFONO del cliente ya lo tenemos (es el número desde el que escribe). NO lo pidas nunca.
- Solo PREGUNTA el nombre si no lo conocemos (el cliente es nuevo). Si el cliente ya existe, salúdalo por su nombre.
- NUNCA menciones IDs al cliente.
- Si el cliente es nuevo, se registrará automáticamente con su número de WhatsApp y el nombre que nos dé.
- IMPORTANTE: Una vez que el cliente te diga su nombre, RECUERDALO para toda la conversación. No preguntes dos veces.`;
}
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            serviceId: { type: 'string', description: 'ID del servicio que devuelve searchService' }
          },
...
async function searchService(query, data) {
  ...
  if (matches.length === 0) return { error: 'No encontré servicios con ese nombre', query };
...
async function checkAvailability(date, serviceId, data) {
  ...
  for (const emp of emps) {
    const appointments = (data.appointments||[]).filter(a =>
      !a._deleted && !a.pendingSalonConfirm && a.date === date && a.employeeId === emp.id
    );
...
async function bookAppointment(args, phone, data) {
  ...
  console.log(` [BOOK-DEBUG] phone="${phone}" clientName="${clientName}" serviceId="${serviceId}" date="${date}" time="${time}"`);
  ...
  const conflict = (data.appointments||[]).some(a =>
    !a._deleted && !a.pendingSalonConfirm && a.date === date && a.employeeId === employeeId && (() => {
  ...
  const appt = {
    ...
    source: 'whatsapp',
    status: 'pending',
    notes: clientName ? 'PENDIENTE - Reservado por WhatsApp por ' + clientName + ' 📱' + client.phone : 'PENDIENTE - Reservado por WhatsApp 📱' + client.phone,
    created: Date.now(),
    _modified: Date.now(),
    _deleted: false,
  ...
  forwardToVercel({ appointments: [appt], clients: [client] });
...
async function httpGetJson(url, timeoutMs = 3000) {
  ...
  return new Promise((resolve, reject) => {
    const req = mod.get(url, res => {
      ...
      res.on('end', () => {
        if (res.statusCode === 200) { try { resolve(JSON.parse(body)); } catch { reject(); } }
...
async function fetchData() {
  ...
  const syncUrls = [VERCEL_SYNC_URL.replace(/\/+$/, ''), 'https://tpv-peluqueria.vercel.app/api', 'https://tpv-peluqueria.vercel.app'];
  for (const base of syncUrls) {
    try {
      const url = base + '/sync';
      console.log(`[AI-DATA] Intentando Vercel: ${url}`);
      const data = await httpGetJson(url, 5000);
      if ((data.services||[]).length > 0) {
        console.log(`[AI-DATA] ${(data.services||[]).length} servicios desde Vercel`);
        return data;
      }
    } catch (e) {
      console.log(`[AI-DATA] Vercel no disponible (${e.message})`);
    }
  }
...
async function processWhatsAppMessage(phone, text) {
  try {
    ...
    if (!DEEPSEEK_API_KEY) return 'Lo siento, no hay IA configurada. Contacta con el salón directamente.';

    const maxTurns = 3;
    for (let turn = 0; turn < maxTurns; turn++) {
      const ctx = [...history.slice(-6)];
      let result = await callDeepSeek([{ role: 'system', content: systemPrompt }, ...ctx], tools);
      if (!result) continue;
      ...
      if (result.tool_calls) {
        ...
        await new Promise(r => setTimeout(r, 2000));
        let secondResult = await callDeepSeek([{ role: 'system', content: systemPrompt }, ...history.slice(-8)], tools);
        if (secondResult && secondResult.content && !secondResult.content.startsWith('Error') && !secondResult.content.startsWith('Lo siento')) {
      ...
      if (result.content) {
        if (result.content.startsWith('Error') || result.content.startsWith('Lo siento') || result.content.startsWith('Uff')) {
          continue;
        }
...
async function processPhoneMessage(phone, text) {
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (result.tool_calls) {
        history.push({ role: 'assistant', content: null, tool_calls: result.tool_calls.map(tc => ({
        ...
        let secondResult = await callDeepSeek([{ role: 'system', content: systemPrompt }, ...history.slice(-8)], tools);
        if (secondResult && secondResult.content && !secondResult.content.startsWith('Error') && !secondResult.content.startsWith('Lo siento')) {
          history.push({ role: 'assistant', content: secondResult.content });
          await saveConversation(phone + ':phone', history);
          return secondResult.content;
        }
        continue;
      }
    }
...
async function forwardToVercel(payload) {
  try {
    ...
    console.log('[FORWARD] Enviando a Vercel:', JSON.stringify({
    ...
    if (res.ok) {
      ...
      console.warn('[FORWARD] Vercel returned', res.status, errText.substring(0, 200));
    }
  } catch (e) {
