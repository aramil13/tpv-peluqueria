...
console.log('[AI-ASSISTANT] Usando Groq. Key=' + (GROQ_API_KEY ? 'SET (' + GROQ_API_KEY.substring(0, 8) + '...)' : 'VACIA'));
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (GROQ_API_KEY) {
        ...
        return '⚠️ No hay API Key de Groq configurada. Ve a Ajustes > IA e introduce tu clave.';
      }
      ...
      if (result.tool_calls) {
        ...
        if (GROQ_API_KEY) {
          ...
          secondResult = { content: '⚠️ No hay API Key de Groq configurada.' };
        }
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (GROQ_API_KEY) {
        ...
        return '⚠️ No hay API Key de Groq configurada. Contacta con el salón directamente.';
      }
