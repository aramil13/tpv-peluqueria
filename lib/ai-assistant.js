...
console.log('[AI-ASSISTANT] Usando SOLO DeepSeek. Key=' + (DEEPSEEK_API_KEY ? 'SET (' + DEEPSEEK_API_KEY.substring(0, 8) + '...)' : 'VACIA'));
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (DEEPSEEK_API_KEY) {
        ...
        return '⚠️ No hay API Key de DeepSeek configurada. Ve a Ajustes > IA e introduce tu clave.';
      }
      ...
      if (result.tool_calls) {
        ...
        if (DEEPSEEK_API_KEY) {
          ...
          secondResult = { content: '⚠️ No hay API Key de DeepSeek configurada.' };
        }
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (DEEPSEEK_API_KEY) {
        ...
        return '⚠️ No hay API Key de DeepSeek configurada. Contacta con el salón directamente.';
      }

      if (result.content) {
        const phoneErrorPrefixes = ['Error', 'Lo siento, el asistente no está configurado', 'Lo siento, no hay IA configurada', 'Uff, estoy muy ocupada', 'Lo siento, hay un problema'];
