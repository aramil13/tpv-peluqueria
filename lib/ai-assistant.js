  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (r.status === 429 || r.status === 413) {
        ...
        console.log(`[GITHUB MODELS RATE-LIMIT] Esperando ${waitMs}ms (intento ${attempt+1}/${retries+1}). Cooldown global 60s hasta ${new Date(rateLimitCooldownUntil).toISOString().slice(11,19)}`);
        if (attempt < retries) {
          await new Promise(r => setTimeout(r, waitMs));
          continue;
        }
