...
function buildSystemPrompt(data, clientInfo) {
  ...
  const empList = (data.employees||[]).filter(e => !e._deleted).map(e => `"${e.name}" (ID: ${e.id})`).join(', ');

  return `Eres Sara, asistente de ${BUSINESS_NAME}. Hoy ${today}. ${hoursStr}.
CLIENTE: ${clientName || 'Nuevo'}
EMPLEADAS DISPONIBLES: ${empList || 'Ninguna'}

REGLAS (no seguirlas = error):
1. SERVICIOS: SOLO searchService("texto") da servicios reales. USA SIEMPRE el nombre exacto que devuelve searchService.
2. TRADUCCION DE HORAS: Si el cliente dice "19" entiende que son las 19:00. Si dice "9 y media" entiende 09:30. Convierte SIEMPRE a formato HH:MM.
3. NOMBRES DE EMPLEADAS: El cliente puede decir nombres parciales. "mar" coincide con "Maria del Mar", "ana" con "Ana", "noe" con "Noemi". USA siempre el nombre completo que ves en EMPLEADAS DISPONIBLES.
4. HORARIOS: SOLO checkAvailability({date, serviceIds}) da huecos. NUNCA digas horas sin llamarla.
5. MULTISERVICIO: UNA checkAvailability con TODOS los IDs. UNA lista, UNA hora, UN empleado.
6. bookAppointment({serviceIds, date, time, employeeId}) con TODOS los IDs. employeeId es el ID de la empleada (de EMPLEADAS DISPONIBLES).
7. NUNCA "reservado"/"confirmado". Siempre "solicitud/pendiente".
8. cancelAppointment({appointmentIds:[id1,id2]}) para varias.
9. NUNCA preguntes teléfono, IDs, ni inventes datos.`;
}
  ...
  return [
    ...
    },
    {
      ...
      function: {
        ...
        description: 'Disponibilidad para VARIOS servicios en UNA CITA. Pasa TODOS los serviceIds juntos en el array.',
        parameters: {
          type: 'object',
          properties: {
            date: { type: 'string', description: 'Fecha YYYY-MM-DD' },
            serviceIds: { type: 'array', items: { type: 'string' }, description: 'IDs de TODOS los servicios (de searchService). Ej: ["id1","id2","id3"]' }
          },
...
async function callDeepSeek(systemPrompt, messages, tools, retries = 3) {
...
const AI_PROVIDERS = [
  { name: 'GitHub', fn: callGitHubModels, key: GITHUB_TOKEN },
...
async function callAI(systemPrompt, messages, tools) {
  ...
  for (const provider of AI_PROVIDERS) {
    ...
    console.log(`[AI] Intentando ${provider.name}...`);
    try {
      const result = await provider.fn(systemPrompt, messages, tools);
      if (result.content && (
        result.content.startsWith('Uff,') ||
        result.content.startsWith('⚠️ El token') ||
        result.content.startsWith('Lo siento, el servicio de IA') ||
        result.content.startsWith('Lo siento, el asistente') ||
        result.content.includes('API Key')
      )) {
        lastError = result.content;
        console.log(`[AI-FALLBACK] ${provider.name} falló: "${result.content.slice(0,60)}". Probando siguiente...`);
        continue;
      }
      return result;
    } catch (e) {
      lastError = e.message;
      console.log(`[AI-FALLBACK] ${provider.name} error: ${e.message}. Probando siguiente...`);
    }
  }
  rateLimitCooldownUntil = Date.now() + 60000;
  return { content: lastError || 'Lo siento, ningún proveedor de IA está disponible ahora.' };
}
...
async function rateLimitWait() {
  ...
  if (now < rateLimitCooldownUntil) {
    const wait = rateLimitCooldownUntil - now;
    console.log(`[RATE-LIMIT] Esperando cooldown global de ${Math.round(wait/1000)}s...`);
...
async function callGitHubModels(systemPrompt, messages, tools, retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (r.status === 429 || r.status === 413) {
        ...
        console.log(`[GITHUB MODELS RATE-LIMIT] Esperando ${waitMs}ms (intento ${attempt+1}/${retries+1})`);
        if (attempt < retries) {
          await new Promise(r => setTimeout(r, waitMs));
          continue;
        }
        return { content: 'Uff, estoy muy ocupada. Dame **1 minuto** y escríbeme de nuevo.' };
      }
      ...
      if (r.status === 400) {
        ...
        return { content: 'Perdona, no entendí bien. ¿Puedes repetirlo?' };
      }
      ...
      return { content: 'Vale, espera un momento y vuelve a decírmelo.' };
    } catch (e) {
...
async function executeTool(toolCall, phone, data) {
  ...
  switch (fn.name) {
    ...
    case 'checkAvailability': {
      ...
      return await checkAvailability(args.date, ids[0], data, ids.slice(1));
    }
...
async function checkAvailability(date, serviceId, data, extraServiceIds) {
  ...
  console.log(`[CHECK-AVAIL] ${entries.length} horarios disponibles para ${date}`);
  const slotList = entries.map(([t, emps]) => {
    const unique = [...new Set(emps)];
    return `${t} - ${unique.join(', ')}`;
  });

  const slotData = entries.map(([t, _]) => {
    const atTime = slots.filter(s => s.time === t);
    return { time: t, employees: atTime.map(s => ({ name: s.employeeName, id: s.employeeId })) };
  });

  const svcIds = ids.join(',');
  return {
    available: true,
    date,
    serviceIds: ids,
...
async function bookAppointment(args, phone, data) {
  ...
  if (ids.length === 0) return { error: 'Debes especificar al menos un servicio' };
  console.log(` [BOOK-DEBUG] phone="${phone}" serviceIds="${ids.join(',')}" date="${date}" time="${time}"`);
...
async function cancelAppointment(appointmentId, phone, data) {
  ...
  try {
    await writeData(data);
    const allPending = (data.appointments||[]).filter(a => a.pendingCancelWhatsApp && !a._deleted);
    await forwardSync({ appointments: allPending, clients: [] });
  } catch (e) {
...
async function cancelMultipleAppointments(appointmentIds, phone, data) {
  ...
  for (const id of appointmentIds) {
    ...
    if (!appt) continue;
    const client = (data.clients||[]).find(c => c.id === appt.clientId);
    if (client && normPhone(client.phone) !== normPhone(phone)) continue;
    const toCancel = appt.blockGroupId
      ? (data.appointments||[]).filter(a => !a._deleted && a.blockGroupId === appt.blockGroupId && a.clientId === appt.clientId)
      : [appt];
    toCancel.forEach(a => {
      if (!allToCancel.find(x => x.id === a.id)) {
        a.pendingCancelWhatsApp = true;
        a.source = a.source || 'whatsapp';
        a._modified = Date.now();
        allToCancel.push(a);
  ...
  if (allToCancel.length === 0) return { error: 'No se pudo cancelar ninguna cita.' };
  try {
    await writeData(data);
    const allPending = (data.appointments||[]).filter(a => a.pendingCancelWhatsApp && !a._deleted);
    await forwardSync({ appointments: allPending, clients: [] });
  } catch (e) {
    console.error('[CANCEL-MULTI-ERROR] Error al guardar cancelación:', e.message);
    allToCancel.forEach(a => { a.pendingCancelWhatsApp = false; a._modified = Date.now(); });
    return { error: 'Hubo un problema al cancelar. Inténtalo de nuevo.' };
  }
  return { success: true, message: `⏳ Solicitud de cancelación enviada para ${allToCancel.length} cita(s). El salón lo revisará y confirmará.` };
}
...
async function fetchData() {
  ...
  const results = await Promise.allSettled(syncUrls.map(base =>
    httpGetJson(base + '/sync', 4000).then(d => ({ source: base, data: d }))
  ));
  for (const r of results) {
    if (r.status === 'fulfilled' && r.value.data?.services?.length > 0) {
      console.log(`[AI-DATA] Cloudflare (${r.value.source}): ${r.value.data.services.length} servicios, ${(r.value.data.appointments||[]).length} citas`);
      return r.value.data;
    }
  }
  console.log(`[AI-DATA] Local: ${(local.services||[]).length} servicios, ${(local.appointments||[]).length} citas`);
  return local;
}
...
async function processWhatsAppMessage(phone, text) {
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      result = await callAI(systemPrompt, ctx, tools);
      ...
      if (result.tool_calls) {
        ...
        secondResult = await callAI(systemPrompt, safeCtx2, tools);
      ...
      if (result.content) {
        const errorPrefixes = ['Error', 'Lo siento,', 'Uff, estoy muy ocupada', 'El servicio de IA', 'Hay un problema', 'Perdona, no entendí'];
...
async function processPhoneMessage(phone, text) {
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (result.content) {
        const phoneErrorPrefixes = ['Error', 'Lo siento,', 'Uff, estoy muy ocupada', 'El servicio de IA', 'Hay un problema', 'Perdona, no entendí', '⚠️ No hay API Key'];
...
async function forwardSync(payload) {
  ...
  fetch(url, {
    ...
    if (res.ok) console.log('[FORWARD] Cita enviada a Cloudflare OK');
    else res.text().then(t => console.warn('[FORWARD] Cloudflare returned', res.status, t.substring(0, 200)));
  }).catch(e => console.warn('[FORWARD] Error (no bloqueante):', e.message));
}
