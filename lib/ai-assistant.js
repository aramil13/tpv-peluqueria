...
REGLAS ESTRICTAS:
 1. INVENTAR SERVICIOS: NUNCA inventes servicios. Usa SOLO los IDs que devuelva searchService. Si buscas y no encuentras, di que no está disponible. searchService devuelve UN ÚNICO servicio, el más adecuado. USA siempre ese.
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (r.status === 400) {
        ...
        console.error('[GITHUB MODELS 400 BODY MSGS]', JSON.stringify(body.messages).substring(0, 1000));
        return { content: 'Lo siento, he tenido un problema interno. Consulta con el salón directamente por teléfono.' };
      }
...
async function searchService(query, data) {
  ...
  const scoreService = (s) => {
    ...
    const nameClean = name.replace(/[^a-z0-9áéíóúüñ ]/g, '').trim();
    if (name === q) return 0;
...
async function bookAppointment(args, phone, data) {
  ...
  const mkAppt = (startMin, blockSvcs, blockNum, blockGroupId) => {
    ...
    return {
      ...
      source: 'whatsapp',
      status: 'pending',
      notes: 'Reserva por Whatsapp - ' + (clientName || 'Cliente WhatsApp') + ' - ' + client.phone + ' - ' + blockSvcs.map(s => s.name.replace(/\.$/, '').trim()).join(', '),
      created: Date.now(),
      _modified: Date.now(),
      _deleted: false,
...
async function cancelAppointment(appointmentId, phone, data) {
  ...
  await writeData(data);
  forwardSync({ appointments: toCancel, clients: [] });
