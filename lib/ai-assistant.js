...
async function processPhoneMessage(phone, text) {
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (result.content) {
        const phoneErrorPrefixes = ['Error Groq', 'Lo siento, el asistente no está configurado', 'Lo siento, no hay IA configurada', 'Uff, estoy muy ocupada', 'Lo siento, hay un problema'];
