...
console.log('[AI-ASSISTANT] Usando Gemini 2.5 Flash. Key=' + (GEMINI_API_KEY ? 'SET (' + GEMINI_API_KEY.substring(0, 8) + '...)' : 'VACIA'));
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (GEMINI_API_KEY) {
        ...
        return '⚠️ No hay API Key de Gemini configurada. Ve a Ajustes > IA e introduce tu clave.';
      }
      ...
      if (result.tool_calls) {
        ...
        if (GEMINI_API_KEY) {
          ...
          secondResult = { content: '⚠️ No hay API Key de Gemini configurada.' };
        }
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (GEMINI_API_KEY) {
        ...
        return '⚠️ No hay API Key de Gemini configurada. Contacta con el salón directamente.';
      }
