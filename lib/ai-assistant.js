...
async function getClientAppointments(phone, data) {
  ...
  console.log(`[GET-APPT] phone=${phone} norm=${normPhone(phone)} clients=${clients.length} totalClientes=${(data.clients||[]).length}`);
  ...
  console.log(`[GET-APPT] Encontradas ${allAppts.length} citas de ${(data.appointments||[]).length} total`);

  if (!allAppts.length) return { appointments: [], message: 'No tienes citas próximas.' };
...
async function fetchData() {
  ...
  for (const port of localPorts) {
    try {
      ...
      if ((data.services||[]).length > 0) {
        console.log(`[AI-DATA] ${(data.services||[]).length} servicios, ${(data.appointments||[]).length} citas desde localhost:${port}`);
        return data;
      }
  ...
  for (const base of syncUrls) {
    try {
      ...
      if ((data.services||[]).length > 0) {
        console.log(`[AI-DATA] ${(data.services||[]).length} servicios, ${(data.appointments||[]).length} citas desde Cloudflare`);
        return data;
      }
    } catch (e) {
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (getGitHubToken()) {
        ...
        return '⚠️ No hay token de GitHub Models configurado. Ve a Ajustes > IA e introduce tu GITHUB_TOKEN.';
      }
      ...
      if (result.tool_calls) {
        ...
        if (getGitHubToken()) {
          ...
          secondResult = { content: '⚠️ No hay token de GitHub Models configurado.' };
        }
