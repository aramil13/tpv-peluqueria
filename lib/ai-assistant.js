...
async function callGroq(messages, tools) {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (r.status === 429) return { content: 'Uff, estoy muy ocupada. Espera un momento y vuelve a escribirme.' };
      return { content: 'Lo siento, hubo un error. Inténtalo de nuevo.' };
    }
...
async function checkAvailability(date, serviceId, data) {
  const settings = data.settings || {};
  const dayCfg = (settings.onlineOpening || {})[date] || {};
  const enabled = dayCfg.enabled !== false;
  if (!enabled) return { available: false, message: 'No hay disponibilidad para ese día (cerrado)' };

  const openTime = dayCfg.time || '09:00';
  const openMin = timeToMinutes(openTime);
  const closeMin = openMin + (dayCfg.maxHours || 10) * 60;
  ...
  return {
    ...
    date,
    slots: slotList,
    message: `Huecos disponibles para ${date}:\n${slotList.join('\n')}`
  };
}
...
async function modifyAppointment(args, phone, data) {
  ...
  const details = [];
  if (newDate || newTime) details.push(`Nuevo horario: ${newDate||appt.date} ${newTime||appt.time}`);
  if (newEmployeeId) {
...
async function getClientAppointments(phone, data) {
  ...
  const list = appts.map(a => {
    ...
    return `🔹 ${a.date} ${a.time}${a.endTime ? '-'+a.endTime : ''} - ${svcName}${empName ? ' con '+empName : ''} (ID: ${a.id})`;
  }).join('\n');
...
function getBusinessInfo(data) {
  ...
  const schedule = settings.onlineOpening ? Object.entries(settings.onlineOpening).slice(0, 7).map(([day, cfg]) => {
    ...
    return `${dayName}: ${cfg.time||'09:00'} - ${cfg.enabled === false ? 'Cerrado' : 'Abierto'}`;
  }).join('\n') : 'Consultar horarios';
  ...
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (result.content) {
        ...
        await sendMessage(phone, result.content);
        return;
      }
      ...
      if (result.tool_calls) {
        ...
        await saveConversation(phone, history);
        await sendMessage(phone, 'No entendí bien. ¿Puedes repetirlo?');
        return;
      }
    }
    ...
    await saveConversation(phone, history);
    await sendMessage(phone, 'La conversación es muy larga. Empecemos de nuevo. ¿Qué necesitas?');

  } catch (e) {
    console.error('[AI ASSISTANT ERROR]', e.message);
    try { await sendMessage(phone, 'Lo siento, hubo un error. Inténtalo de nuevo o llama al '+BUSINESS_PHONE); } catch {}
  }
}
