...
function normalizeDate(d) {
  ...
  // If already YYYY-MM-DD and valid, return as-is
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return d.trim();
  ...
  // "mañana 28" or "manana 28" -> tomorrow's day is 28
  m = s.match(/^ma(?:ñana|nana)\s+(\d{1,2})$/);
  if (m) {
  ...
  // "27 de mayo" or "27 mayo" or "mayo 27"
  for (const [name, num] of Object.entries(MONTHS)) {
    m = s.match(new RegExp('(\\d{1,2})\\s*de?\\s*'+name));
    if (m) {
      const y = new Date().getFullYear();
      return y+'-'+String(num).padStart(2,'0')+'-'+m[1].padStart(2,'0');
    }
    m = s.match(new RegExp(name+'\\s*(\\d{1,2})'));
    if (m) {
      const y = new Date().getFullYear();
      return y+'-'+String(num).padStart(2,'0')+'-'+m[1].padStart(2,'0');
    }
...
function buildSystemPrompt(data, clientInfo) {
  const services = (data.services||[]).filter(s => !s._deleted).map(s =>
    `- ${s.name} (${s.duration||30} min${s.price ? ', '+s.price+'€' : ''})`
  ).join('\n');
  ...
  for (let attempt = 0; attempt < retries; attempt++) {
    try {
      ...
      if (!r.ok) {
        ...
        console.error('[GROQ ERROR]', r.status, errText.substring(0, 200));
        if (r.status === 429) {
          let waitMs = 5000;
          try {
            const errJson = JSON.parse(errText);
            if (errJson.error?.retry_after) {
              waitMs = Math.ceil(parseFloat(errJson.error.retry_after) * 1000);
            } else {
          ...
          if (attempt < retries - 1) {
            console.log(`[GROQ] 429, esperando ${waitMs}ms antes de reintentar ${attempt + 1}/${retries}`);
            await new Promise(r => setTimeout(r, waitMs));
            continue;
          }
          return { content: 'Estoy algo saturada ahora mismo. Dame unos segundos y vuelve a escribirme 😊' };
        }
...
async function executeTool(toolCall, phone, data) {
  ...
  if (args.date) args.date = normalizeDate(args.date);
  if (args.appointmentDate) args.appointmentDate = normalizeDate(args.appointmentDate);
...
async function checkAvailability(date, serviceId, data) {
  const settings = data.settings || {};
  const onlineOpening = settings.onlineOpening || {};
  const schedule = settings.schedule || {};
  
  const dayCfg = onlineOpening[date];
  const dayOfWeek = new Date(date + 'T00:00:00').getDay();
  const openDays = schedule.openDays || [1,2,3,4,5];
  
  const openTime = dayCfg?.time || schedule.openTime || '09:00';
  const closeTime = schedule.closeTime || '19:00';
  const maxHours = dayCfg?.maxHours || schedule.maxHours || 10;
  
  let enabled = dayCfg?.enabled !== false;
  if (!dayCfg && schedule.openDays) {
    enabled = openDays.includes(dayOfWeek);
  }
  
  console.log('[CHECK] Date:', date, 'DayOfWeek:', dayOfWeek, 'OpenDays:', openDays);
  console.log('[CHECK] Day config:', JSON.stringify(dayCfg), 'Schedule:', JSON.stringify(schedule));
  
  if (!enabled) {
    return { 
      available: false, 
      message: `El ${formatDate(date)} no está disponible para reservas online. Por favor, llama al salón para consultar disponibilidad.` 
    };
  }

  const openMin = timeToMinutes(openTime);
  const closeMin = openMin + maxHours * 60;
  ...
  if (slots.length === 0) return { 
    available: false, 
    message: `Para el ${formatDate(date)} no hay huecos disponibles en este momento. ¿Prefieres otro día?` 
  };
  ...
  return {
    ...
    date,
    slots: slotList,
    message: `Huecos disponibles para ${formatDate(date)} (${openTime} - ${closeTime}):\n${slotList.join('\n')}`
  };
}
...
function getBusinessInfo(data) {
  const settings = data.settings || {};
  const services = (data.services||[]).filter(s => !s._deleted).map(s =>
    `• ${s.name}${s.price ? ' - '+s.price+'€' : ''} (${s.duration||30} min)`
  ).join('\n');
