...
async function callGitHubModels(systemPrompt, messages, tools, retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (r.status === 400) {
        console.error('[GITHUB MODELS 400] Mensajes enviados:', JSON.stringify(body.messages.slice(0, 3)).substring(0, 500));
        return { content: 'Error de comunicación con la IA. Inténtalo de nuevo.' };
      }
