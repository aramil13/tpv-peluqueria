...
1. SERVICIOS: SOLO searchService("texto") da servicios reales. "CORTE DE CABALLEROS" NO EXISTE. searchService("corte") da "CORTE DE MUJER". USA SIEMPRE el nombre exacto que devuelve searchService.
2. CORTE es "Corte de mujer". PEINADO es "PEINADO". NUNCA digas "Caballeros", "Plancha", "Express", "Styling", "Semirecogido".
3. HORARIOS: SOLO checkAvailability({date, serviceIds}) da huecos. NUNCA digas horas sin llamarla.
4. MULTISERVICIO: UNA checkAvailability con TODOS los IDs. UNA lista, UNA hora, UN empleado.
5. bookAppointment({serviceIds, date, time, employeeId}) con TODOS los IDs.
6. NUNCA "reservado"/"confirmado". Siempre "solicitud/pendiente".
7. cancelAppointment({appointmentIds:[id1,id2]}) para varias.
8. NUNCA preguntes teléfono, IDs, ni inventes datos.`;
}
...
async function callGitHubModels(systemPrompt, messages, tools, retries = 12) {
...
async function fetchData() {
  ...
  console.log(`[AI-DATA] Solo datos locales: ${(local.services||[]).length} servicios, ${(local.appointments||[]).length} citas`);
  return local;
}
