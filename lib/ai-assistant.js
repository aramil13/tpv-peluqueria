...
console.log('[AI-ASSISTANT] Usando GitHub Models (Llama 3.3 70B). Token=' + (GITHUB_TOKEN ? 'SET (' + GITHUB_TOKEN.substring(0, 8) + '...)' : 'VACIA'));
const GROQ_MODEL = 'llama-3.3-70b-versatile';
const GITHUB_MODEL = 'Meta-Llama-3.1-8B-Instruct';
...
async function callAI(systemPrompt, messages, tools, retries) {
  if (GROQ_API_KEY) return await callGroq(systemPrompt, messages, tools, retries);
  if (getGitHubToken()) return await callGitHubModels(systemPrompt, messages, tools, retries);
  if (GEMINI_API_KEY) return await callGemini(systemPrompt, messages, tools, retries);
  return { content: '⚠️ No hay IA configurada. Ve a Ajustes > IA e introduce una API key.' };
}

async function callGitHubModels(systemPrompt, messages, tools, retries = 5) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (r.status === 400) {
        ...
        return { content: 'Error de IA (400): '+errText.substring(0, 120) };
      }

      return { content: 'Error GitHub Models ('+r.status+'): '+errText.substring(0,100) };
    } catch (e) {
...
async function getClientAppointments(phone, data) {
  ...
  const list = allAppts.map(a => {
    ...
    return `🔹 ${formatDate(a.date)} ${a.time}${a.endTime ? '-'+a.endTime : ''} - ${svcName}${empName ? ' con '+empName : ''}${pend}${canc} (ID: ${a.id})`;
  }).join('\n');
...
async function fetchData() {
  ...
  for (const port of localPorts) {
    try {
      ...
      if ((data.services||[]).length > 0) {
        data._source = `localhost:${port}`;
        if (!best || (data.services||[]).length > (best.services||[]).length) best = data;
      }
    } catch (e) {}
  }
  ...
  for (const base of syncUrls) {
    try {
      ...
      if ((data.services||[]).length > 0) {
        data._source = 'Cloudflare';
        if (!best || (data.services||[]).length > (best.services||[]).length) best = data;
      }
    } catch (e) {}
  }
  if (best) {
    console.log(`[AI-DATA] Usando fuente: ${best._source} (${(best.services||[]).length} servicios, ${(best.appointments||[]).length} citas)`);
    return best;
  }
