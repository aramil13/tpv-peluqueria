...
async function callGitHubModels(systemPrompt, messages, tools, retries = 3) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const errText = await r.text();
      console.error('[GITHUB MODELS ERROR]', r.status, errText);
      ...
      if (r.status === 401) {
        console.error('[GITHUB MODELS 401] Token inválido. Crea un token en GitHub > Settings > Developer settings > Personal access tokens > Fine-grained tokens con permiso Models:Read');
        return { content: '⚠️ El token de GitHub no es válido. Sigue estos pasos:\n1. Ve a https://github.com/settings/tokens\n2. Clica "Generate new token" > "Fine-grained token"\n3. Permiso: Models > Read\n4. Copia el token y pégalo en Ajustes > IA > GitHub Token' };
      }
