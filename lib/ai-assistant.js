...
- NUNCA incluyas etiquetas XML o <function> en tu respuesta. Siempre usa las herramientas correctamente.`;
}
...
async function processWhatsAppMessage(phone, text) {
  try {
    ...
    for (let turn = 0; turn < maxTurns; turn++) {
      ...
      if (result.content) {
        const clean = result.content.replace(FUNCTION_CALL_RE, '').trim() || 'Entendido. ¿Qué más necesitas?';
