...
- getMyAppointments(): muestra citas del cliente.
...
async function checkAvailability(date, serviceId, data, extraServiceIds) {
  ...
  // Limitar a 30 huecos para evitar payload enorme (400 error)
  const limited = slots.slice(0, 30);
  const grouped = {};
  limited.forEach(s => {
    const key = s.time;
...
async function bookAppointment(args, phone, data) {
  ...
  if (hasBlocks) {
    ...
    forwardToVercel({ appointments: createdAppts, clients: [client] });
