...
<#
.SYNOPSIS
  Detalla fila a fila (Detalle_Factura_Anulaciones) todas las ventas eliminadas sin cobrar
  (Cabecera_Factura_Anulaciones) entre dos fechas, agrupadas por numero de factura,
  con desglose Base / IVA / Total por factura y resumen global.

.USO
  .\buscar-ventas-anuladas.ps1 -Desde 2026-08-01 -Hasta 2026-08-21
  .\buscar-ventas-anuladas.ps1                       # pedira las fechas
  .\buscar-ventas-anuladas.ps1 -Desde ... -Hasta ... -ExportarCsv "$env:USERPROFILE\Desktop\anuladas.csv"
  .\buscar-ventas-anuladas.ps1 -Desde ... -Hasta ... -Json   # salida JSON
#>
param(
  [datetime]$Desde,
  [datetime]$Hasta,
...
$cabs = Invoke-Query "SELECT Numero, Fecha, Hora, Cliente, Empleado, total, ImpuestoIncluido, fechaBorrado FROM [Cabecera_Factura_Anulaciones] WHERE Fecha >= ? AND Fecha < ? ORDER BY Fecha, Numero" @($Desde.Date, $Hasta.Date.AddDays(1))

if (-not $cabs.Rows.Count) {
  if ($Json) {
    Write-Output ([pscustomobject]@{ ok = $true; desde = $Desde.ToString('yyyy-MM-dd'); hasta = $Hasta.ToString('yyyy-MM-dd'); ventas = @(); resumen = [pscustomobject]@{ n = 0; base = 0; iva = 0; total = 0 } } | ConvertTo-Json -Depth 6)
    $conn.Close(); exit 0
  }
  Write-Host ("No hay ventas eliminadas entre {0:dd/MM/yyyy} y {1:dd/MM/yyyy}" -f $Desde, $Hasta) -ForegroundColor Yellow
  $conn.Close(); exit 0
}

$gBase = 0.0; $gIva = 0.0; $gTotal = 0.0; $csvRows = @(); $ventas = @()
...
foreach ($cab in $cabs.Rows) {
  ...
  if (-not $Json) {
    Write-Host ''
    Write-Host ('=' * 78)
    Write-Host ("Venta ELIMINADA #{0}  |  {1:dd/MM/yyyy HH:mm}  |  {2}  |  {3}" -f $num, $cab.Fecha, $clienteTxt, $cab.Empleado) -ForegroundColor Cyan
    if ($incl) { Write-Host '(precios con IVA incluido)' -ForegroundColor DarkGray }
  }

  $det = Invoke-Query "SELECT Codigo, Producto, Unidades, Precio_Unit, Descuento, TipoImpuesto, NombreEmpleado, FechaAnulacion FROM [Detalle_Factura_Anulaciones] WHERE Num_Factura = ? ORDER BY Codigo" @($num)

  if (-not $det.Rows.Count) {
  ...
  foreach ($l in $det.Rows) {
    ...
    if (-not $Json) {
      Write-Host ("  {0,-38} {1,7:n2} {2,10:n2} {3,5:n0} {4,5:n0} {5,10:n2}" -f $nombreCorto, $u, $pu, $rate, $dto, $sub)
    ...
    $lineasArr += [pscustomobject]@{
      ...
      base       = [math]::Round($base, 2)
      empleada   = [string]$l.NombreEmpleado
      anuladaEl  = $(if ($l.FechaAnulacion -is [datetime]) { $l.FechaAnulacion.ToString('dd/MM/yyyy') } else { '' })
    }
  }
  ...
  if (-not $Json) {
    ...
    Write-Host ("  TOTAL  Base: {0:n2} EUR  |  IVA: {1:n2} EUR  |  Total venta: {2:n2} EUR" -f $totBase, $totIva, $totalCab) -ForegroundColor Green
    if ([math]::Abs($dif) -ge 0.02) { Write-Host ("  AVISO: diferencia de {0:n2} EUR entre lineas y cabecera" -f $dif) -ForegroundColor Yellow }
  }

  $ventas += [pscustomobject]@{
    numero     = $num
    fecha      = ([datetime]$cab.Fecha).ToString('dd/MM/yyyy')
    cliente    = $clienteTxt
...
Write-Host ''
Write-Host ('=' * 78)
Write-Host ("RESUMEN: {0} ventas eliminadas sin cobrar | Base: {1:n2} EUR | IVA: {2:n2} EUR | Total: {3:n2} EUR" -f $cabs.Rows.Count, $gBase, $gIva, $gTotal) -ForegroundColor Cyan
Write-Host ('=' * 78)
